  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="pt-24 pb-12 relative overflow-hidden">
        ...
        <div className="container relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
            <Link to="/#core-products" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-primary transition-colors">
            ...
            <motion.div ref={ref} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              ...
              <div className="flex flex-wrap gap-2">
                {["开箱即用", "静音运行", "无需机房", "一键部署"].map((tag, i) => (
                  <span key={tag} className="px-3 py-1.5 rounded-full text-xs bg-secondary text-secondary-foreground">
      ...
      <section className="py-12 bg-secondary/30">
        <div className="container">
          <div className="text-center mb-8">
            <h2 className="text-2xl md:text-3xl font-bold mb-2">{t("产品系列", "Product Series")}</h2>
            <p className="text-sm text-muted-foreground">{t("从工作站到机架服务器，满足不同规模", "From workstation to rack server")}</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {products.map((product, index) => (
              ...
              >
                ...
                <div className="p-5">
                  <span className="text-xs text-primary font-medium">{product.subtitleZh}</span>
                  <h3 className="text-lg font-bold mb-1">{t(product.titleZh, product.titleEn)}</h3>
                  <p className="text-xs text-muted-foreground mb-3">{t(product.modelZh, product.modelEn)}</p>
                  <p className="text-sm text-muted-foreground mb-4">{t(product.descZh, product.descEn)}</p>

                  <div className="grid grid-cols-2 gap-2 mb-4">
                    {product.specs.map((spec) => (
                      <div key={spec.labelZh} className="p-2 rounded-lg bg-secondary/50 text-center">
                        <p className="text-[10px] text-muted-foreground">{t(spec.labelZh, spec.labelEn)}</p>
                        <p className="text-xs font-medium">{t(spec.valueZh, spec.valueEn)}</p>
                      </div>
                    ))}
                  </div>
                  ...
                  {product.performance.length > 0 && (
                    <div className="flex flex-wrap gap-2 pt-3 border-t border-border/50">
                      {product.performance.map((perf) => (
                        <span key={perf.labelZh} className="text-xs">
      ...
      <section className="py-12">
        <div className="container">
          <div className="text-center mb-8">
            <h2 className="text-2xl md:text-3xl font-bold mb-2">{t("核心优势", "Core Advantages")}</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {advantages.map((adv, index) => (
              ...
              >
                ...
                <h3 className="text-sm font-semibold mb-1">{t(adv.titleZh, adv.titleEn)}</h3>
                <p className="text-xs text-muted-foreground">{t(adv.descZh, adv.descEn)}</p>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div className="flex flex-wrap justify-center gap-8 md:gap-16">
            {liquidCooling.map((item) => (
              <div key={item.labelZh} className="text-center">
                ...
                <p className="text-2xl font-bold">{t(item.valueZh, item.valueEn)}</p>
                <p className="text-xs text-muted-foreground">{t(item.labelZh, item.labelEn)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-12">
        <div className="container">
          <div className="text-center mb-6">
            <h2 className="text-2xl md:text-3xl font-bold mb-2">{t("AI应用生态", "AI App Ecosystem")}</h2>
            <p className="text-sm text-muted-foreground">{t("内置AI应用商店，一键安装", "Built-in AI app store")}</p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {aiApps.map((app) => (
              <div key={app.zh} className="p-3 rounded-lg bg-primary/5 border border-primary/10 text-center text-sm font-medium">
                {t(app.zh, app.en)}
              </div>
            ))}
          </div>
      ...
      <section className="py-12 bg-secondary/30">
        <div className="container">
          <div className="text-center mb-6">
            <h2 className="text-2xl md:text-3xl font-bold">{t("本地部署 vs 云服务", "Local vs Cloud")}</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[500px] text-sm">
              <thead>
                <tr className="border-b border-border">
                  <th className="py-3 px-4 text-left">{t("对比维度", "Comparison")}</th>
                  <th className="py-3 px-4 text-left bg-primary/10">
                    <div className="flex items-center gap-2">
                      <Server className="w-4 h-4 text-primary" />
                      {t("本地部署", "Local")}
                    </div>
                  </th>
                  <th className="py-3 px-4 text-left">{t("云服务", "Cloud")}</th>
                </tr>
              </thead>
              <tbody>
                {comparison.map((row) => (
                  <tr key={row.aspect.zh} className="border-b border-border/50">
                    <td className="py-3 px-4 font-medium">{t(row.aspect.zh, row.aspect.en)}</td>
                    <td className="py-3 px-4 bg-primary/5">
                      <div className="flex items-center gap-2">
                        <CheckCircle2 className="w-4 h-4 text-primary shrink-0" />
                        {t(row.local.zh, row.local.en)}
                      </div>
                    </td>
                    <td className="py-3 px-4 text-muted-foreground">{t(row.cloud.zh, row.cloud.en)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="py-12">
        <div className="container">
          <div className="text-center mb-6">
            <h2 className="text-2xl md:text-3xl font-bold mb-2">{t("适用客户", "Target Customers")}</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {customers.map((c, index) => (
              ...
              >
                ...
                <h3 className="font-semibold mb-1">{t(c.zh, c.en)}</h3>
                <p className="text-xs text-muted-foreground">{t(c.descZh, c.descEn)}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-12 bg-gradient-to-b from-transparent via-primary/5 to-transparent">
        <div className="container">
          <div className="text-center max-w-xl mx-auto">
            <h2 className="text-2xl md:text-3xl font-bold mb-4">{t("开启企业AI私有化之路", "Start Your AI Journey")}</h2>
            <p className="text-muted-foreground mb-6">{t("数据安全、成本可控、开箱即用", "Secure, cost-effective, plug & play")}</p>
